...
### EARS
- docs/specs/commands-specs.md (35 active specs: 7 implemented, 28 gaps; 4 deferred)
...
### Code
- src/main.rs — clap command dispatch; stubs for init/start/block/unblock/done
...
**Purpose:** All VAT commands except `vat sync`. Config commands are fully implemented. Other commands are scaffolded (argument types wired in clap) but return "not yet implemented" from their bodies.

**Key Components:**
1. `src/main.rs` — dispatch for all commands; stubs for init/start/block/unblock/done
2. `src/cmd_config.rs` — full `vat config get/set` logic
3. `src/readme_template.rs` — `BACKLOG_README_TEMPLATE` rendered with project prefix on init
...
| `vat init` | CMD-INIT-001 to 007 | 0 | 0 | 7 |
...
**Summary:** 7 of 35 active specs implemented (CMD-CC-001, CMD-CFG-001 to 006); 4 deferred; 28 gaps.
...
### Must Fix
1. Implement `vat init` (CMD-INIT-001 to 007) — largely unblocked (readme_template exists; needs `backlog_file` write path and directory creation logic).
2. Implement `find_entry` shared helper — blocked on FMT-MARK-* from `backlog-format`.
3. Implement `start`, `block`, `unblock`, `done` using `find_entry` once it exists.
