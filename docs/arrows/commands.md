...
**PARTIAL** — last audited 2026-06-13. Config commands (CMD-CFG-001 to 006), `vat init` (CMD-INIT-001 to 007), `vat start` (CMD-START-001 to 004), `vat unblock` (CMD-UNBLOCK-001 to 002), and `vat done` (CMD-DONE-001 to 005) are fully implemented with tests, as is the cross-cutting layer (CMD-CC-001 to 004). Exit-code framework (CMD-EXIT-001 to 003) implemented and tested, and now wired through `cmd_config`, `cmd_start`, `cmd_unblock`, and `cmd_done` via `classify_exit_code` (see Key Finding #5). Only `vat block` remains a stub.
...
### Code
- src/main.rs — clap command dispatch; `cmd_init`/`cmd_start`/`cmd_unblock`/`cmd_done` dispatchers; stub for block; `classify_exit_code()` (CMD-EXIT-001 to 003)
...
**Purpose:** All VAT commands except `vat sync`. Config, `vat init`, `vat start`, `vat unblock`, and `vat done` are fully implemented. `block` is scaffolded (argument types wired in clap) but returns "not yet implemented" from its body.

**Key Components:**
1. `src/main.rs` — dispatch for all commands; stub for block; `prompt_for_prefix()` for CMD-INIT-003
...
5. `src/cmd_unblock.rs` — full `vat unblock` logic (strip `[blocked-by:...]`, or no-op when absent), reusing `find_entry_index` and the shared emitter
6. `src/cmd_done.rs` — full `vat done` logic (entry removal + auto-unblock), reusing `find_entry_index` and the bullet/region machinery
7. `src/readme_template.rs` — `BACKLOG_README_TEMPLATE` rendered with project prefix on init
...
**Summary:** 30 of 37 active specs implemented (CMD-CC-001 to 004, CMD-INIT-001 to 007, CMD-START-001 to 004, CMD-UNBLOCK-001 to 002, CMD-DONE-001 to 005, CMD-CFG-001 to 006, CMD-EXIT-001 and 002); 4 deferred; 7 gaps (`block` 6 and CMD-EXIT-003). Note: `commands-specs.md` marks CMD-EXIT-003 `[x]`, but it is counted as a gap here because `cmd_init` and `cmd_sync` exit 1 for internal errors (see Key Finding #5 and the `commands` drift entry in `index.yaml`).
...
4. **Only `block` remains a stub** — `src/main.rs` returns "not yet implemented" for `vat block`; it needs the same `find_entry_index` helper (already available) plus marker add/replace logic. `start`, `unblock`, and `done` are all fully implemented on top of that helper and the FMT-MARK-* bullet machinery. `vat unblock` (`src/cmd_unblock.rs`) clears the `[blocked-by:...]` field and re-serializes via `serialize_region_with_replaced_bullet` (CMD-UNBLOCK-002), or no-ops without writing when there is no blocker (CMD-UNBLOCK-001); both `cmd_unblock` and `cmd_start` reuse the `Bullet` already parsed by `find_entry_index` (carried on `EntryLookup::Found`). The interim-shared emitter is homed in `cmd_start`; vat-m2k will consolidate it.

5. **Exit-code framework (CMD-EXIT-001 to 003) implemented for cmd_config** — `classify_exit_code()` at `src/main.rs:183` chain-searches the anyhow error for typed variants (`ConfigError`, `UserConfigError`, `UnsupportedVersion`, `UserError`) and maps them to exit 1 (user-facing) or 2 (internal). `UserError` in `src/errors.rs` lifts untyped `bail!` messages into the classification scheme. 17 unit tests in `src/main.rs`; all three exit codes are `@spec`-annotated. **Partial coverage:** `classify_exit_code` is wired through `cmd_config_get`/`cmd_config_set`, `cmd_start`, `cmd_unblock`, and `cmd_done`; `cmd_init` and `cmd_sync` still hardcode exit 1 for all errors. `cmd_unblock` additionally wraps its backlog read/write IO in `UserError` so a missing/unreadable backlog exits 1 (user-facing) rather than 2.

## Work Required

### Must Fix
1. Implement `block` using the existing `find_entry_index` helper and the `Bullet` marker add/replace path.

### Should Fix
2. Thread `classify_exit_code` through `cmd_init` and `cmd_sync` so CMD-EXIT-003 applies to every command, not just `cmd_config`/`cmd_start`/`cmd_unblock`/`cmd_done` operations.
