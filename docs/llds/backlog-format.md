...
- A duplicate of a single-occurrence marker (`[id]`, `[in-progress]`, `[by:...]`) is treated like an unknown token: marker parsing stops and the duplicate starts the title, keeping hand-typed repetition visible. `[blocked-by:...]` is the deliberate exception — users plausibly list multiple blockers, so extras are consumed and discarded with only the first kept (FMT-MARK-007). The discarded target IDs are surfaced to the caller (`Bullet::parse_reporting_dropped`) rather than dropped silently, so a re-serializing caller such as `vat sync` can warn the user (SYNC-MARK-004).
...
**Prefix validator (`src/prefix.rs`).** A separate `validate(s) -> Result<(), PrefixError>` for the **project-ID prefix** (FMT-PFX-001..004). It accepts any 3 ASCII *alphanumeric* characters (`is_ascii_alphanumeric`: letters `a`–`z` case-insensitive, digits `0`–`9`), and rejects the `-` separator, whitespace, and `[`/`]` brackets so `[<prefix>-<suffix>]` ID tokens keep parsing. The prefix is chosen once by a human and never regenerated, so the Crockford readability constraint (no `I`/`L`/`O`/`U`) is the wrong rule — it needlessly rejects natural choices like `lib`, `ui`, `io`, `sql`. We deliberately go alphanumeric rather than letters-only: digits cost nothing to allow, parse unambiguously in the `<prefix>-<suffix>` token, and let users mirror existing project codes (`k8s`, `v2x`, `s3`). It is wired into `ProjectConfig` (`vat init` / `vat config set project.id`), the tombstone reader/writer (`.used-ids` prefix segment), and the bullet `[id]`/`[blocked-by:]` marker parser — every place a `<prefix>-<suffix>` token's prefix segment is checked — so a relaxed prefix round-trips end to end.

**Error type.** The prefix validator owns a dedicated `PrefixError` (thiserror) rather than reusing `Base32Error`: the prefix is *not* Crockford base32, so a "base32"-named error type and message would mislabel the failure (a rejected `a-b` is "must be ASCII alphanumeric", not "invalid base32 character"). `PrefixError` mirrors `Base32Error`'s shape — the same `WrongLength { expected, got }` / `InvalidChar { ch, pos }` split, with `pos` a 0-based char index — so `ProjectConfig`'s `InvalidProjectId(#[from] PrefixError)` and the exit-code classification in `main.rs` work the same way; only the wording differs. The suffix validator's `Base32Error` is the parallel type with identical variant shape. `pos` aligns with printed glyph positions even for non-ASCII input; a renderer can `+1` for a 1-based "column N" message and match on `InvalidChar` to render a caret under the bad character. The project-wide error-handling pattern is documented in [cli.md](./cli.md#error-handling).
