...
- [ ] **SYNC-ID-001** — When `vat sync` encounters a bullet without an `[id]` marker, the system shall assign it a new ID composed of the project prefix from `vat.toml`, a literal `-`, and 3 randomly-generated Crockford base32 characters.
- [ ] **SYNC-ID-002** — When generating a new ID, the system shall reject any candidate that appears in `backlog/.used-ids` or that is currently present on another bullet in the parsed region, and retry up to 100 times.
- [ ] **SYNC-ID-003** — When ID generation exhausts its retry cap, the system shall abort with an error and shall not write to any file.
...
- [ ] **SYNC-ID-005** — When `vat sync` encounters a bullet whose `[id]` prefix does not match the configured `project.id`, the system shall print a warning and leave the marker unchanged.
- [ ] **SYNC-ID-006** — When `vat sync` encounters two bullets sharing the same `[id]`, the system shall abort with an error and shall not write to any file.
...
- [ ] **SYNC-WRITE-002** — When the serialized output of `vat sync` is byte-identical to the input file, the system shall skip the write to `backlog.md`.
...
- [ ] **SYNC-WRITE-004** — When `vat sync` runs and `backlog/items/` does not exist but a write is needed, the system shall create it.

## Preconditions

- [ ] **SYNC-PRE-001** — When `backlog/backlog.md` does not exist, `vat sync` shall abort with an error pointing the user at `vat init`.
- [ ] **SYNC-PRE-002** — When the `backlog.md` frontmatter `version` exceeds the CLI's supported major version, `vat sync` shall abort before any other processing.
